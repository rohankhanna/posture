...
DB B -> assert the five MAP tables (cves, crosswalk, candidates, distrust_marks,
seen_cves) are identical. The spine is the MAP; territory (verdicts) must never
...
def _seed(conn) -> None:
    ...
    distrust_marks + seen_cves, across two published months + an unknown."""
...
def test_export_round_trip_identical(conn, tmp_path):
    ...
    # trip (import uses a full INSERT OR REPLACE, not upsert_cve which drops them)
    cve = store.get_cve(other, "CVE-2026-1001")
...
def test_manifest_sha256_is_tamper_evident(conn, tmp_path):
    ...
    out = tmp_path / "out"
    export.export_spine(conn, out_dir=out, policy_version="v")
    shard = out / "spine" / "cves" / "2026-07.jsonl"
