"""Defect catalog store tests — defects/seen_defects/state + the no-wipe per-key
...
# --- defects catalog + provenance ---------------------------------------------

def test_upsert_defect_inserts_with_provenance_and_discovered_at(conn):
    ...
    assert row["enrich_state"] is None  # upsert_defect does NOT set enrich_state
...
def test_upsert_defect_preserves_enrich_state_and_discovered_at_on_re_upsert(conn):
    store.upsert_defect(conn, {"id": "CVE-2026-1", "published": "2026-08-01",
                            "description": "skeleton", "fixed_raw": {"source": "mitre"},
                            "refs": [], "source": "mitre", "fetched_at": "t1",
                            "policy_version": "v", "complete": 1})
    store.set_enrich_state(conn, "CVE-2026-1", "mitre")
    first_seen = store.get_defect(conn, "CVE-2026-1")["discovered_at"]
    ...
    store.upsert_defect(conn, {"id": "CVE-2026-1", "published": "2026-08-01",
                            ...
                            "policy_version": "v", "complete": 1})
    row = store.get_defect(conn, "CVE-2026-1")
    # enrich_state preserved (upsert_defect does NOT touch it) — set_enrich_state must.
...
def test_set_enrich_state_and_pending_enrichment_ids(conn):
    for i in range(3):
        store.upsert_defect(conn, {"id": f"CVE-2026-{i}", "published": f"2026-08-0{i}",
...
def test_catalog_list_filters_by_enrich_state(conn):
    store.upsert_defect(conn, {"id": "CVE-A", "published": "2026-08-01", "description": "",
                            "fixed_raw": {}, "refs": [], "source": "mitre",
                            "fetched_at": "t", "policy_version": "v", "complete": 1})
    store.set_enrich_state(conn, "CVE-A", "mitre")
    store.upsert_defect(conn, {"id": "CVE-B", "published": "2026-08-02", "description": "",
...
def test_mark_defect_distrust_marks_not_deletes(conn):
    store.upsert_defect(conn, {"id": "CVE-X", "published": "2026-08-01", "description": "",
                            "fixed_raw": {}, "refs": [], "source": "nvd",
                            "fetched_at": "t", "policy_version": "v", "complete": 1})
    assert store.mark_defect_distrust(conn, "CVE-X", "audit") is True
    row = store.get_defect(conn, "CVE-X")
    assert row["distrusted"] == 1 and row["distrust_reason"] == "audit"
    # row retained (not deleted), re-mark is a no-op
    assert store.mark_defect_distrust(conn, "CVE-X", "again") is False
    assert store.get_defect(conn, "CVE-X") is not None


# --- seen_defects --------------------------------------------------------------

def test_mark_seen_returns_only_newly_seen(conn):
    store.upsert_defect(conn, {"id": "CVE-1", "published": "2026-08-01", "description": "",
                            "fixed_raw": {}, "refs": [], "source": "mitre",
                            "fetched_at": "t", "policy_version": "v", "complete": 1})
