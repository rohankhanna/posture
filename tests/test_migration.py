"""Migration tests — the in-place schema migrations (v0 -> v1 -> v2 -> v3).
...
and (c) touch ONLY crosswalk/cves/flaws/seen_*/candidates — verdicts (territory)
are byte-untouched.
"""
import sqlite3
...
def test_fresh_db_is_already_at_v3():
    """A fresh db created with the current SCHEMA is at user_version 3 after
    connect; v0->v1 + v1->v2 are safe no-ops on it, and v3 raises the candidates
    unique index (a no-op dedup on an empty table)."""
