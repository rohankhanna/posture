"""Tests for the CISA KEV overlay threat observer — the first REAL observer on
the threat axis.
...
These pin five things:
  ...
  2. the observer emits honest verdicts from inline device inputs (cve_candidates
     + kev) and from a ``kev_path`` pointing at the bundled fixture, with
     provenance wired (observer == "kev", raw_ref = kev:<cve>);
  3. the observer is an honest no-op when the device gives no cve_candidates;
  ...
  5. in the engine, the threat axis becomes REAL: ``targeted`` when any CVE is
     KEV-listed, ``clear`` when none are, ``unknown`` when the observer no-ops,
     and the committed per-verdict rows attribute to observer "kev".
...
from posture.sources.kev_observer import KevThreatObserver
...
dated: 2026-08-06
rationale: |
  test policy for the kev threat observer (self-contained test).
observers:
  kev:
    axes: [threat]
    weight: high
...
def _registry() -> ObserverRegistry:
    ...
    reg.register(KevThreatObserver())
    return reg


# ---------------------------------------------------------------------------
# observer (offline): targeted / clear / false-safe no-op
# ---------------------------------------------------------------------------

def test_observer_targeted_for_kev_cve_and_clear_for_the_rest():
    w = KevThreatObserver()
    pol = _policy()
    device = {
        "id": "host",
    ...
    for v in result.verdicts:
        ...
        assert v.provenance.observer == "kev"
        assert v.provenance.raw_ref == f"kev:{v.key}"


def test_observer_empty_kev_set_means_all_clear():
...
def test_observer_kev_path_reads_fixture_file():
    w = KevThreatObserver()
    pol = _policy()
    device = {
        "id": "host",
...
def test_observer_kev_path_bare_filename_falls_back_to_fixture_dir():
    w = KevThreatObserver()
    pol = _policy()
...
def test_observer_no_candidates_is_honest_noop():
...
def test_observer_candidates_without_overlay_is_false_safe_noop():
...
def test_observer_missing_kev_path_is_complete_zero_not_failure():
    w = KevThreatObserver()
    pol = _policy()
...
def test_observer_inline_kev_takes_precedence_over_path():
    w = KevThreatObserver()
    pol = _policy()
...
def test_engine_threat_axis_targeted_when_any_kev_and_attributed_rows():
    ...
    assert rows["CVE-2026-1001"]["observer"] == "kev"
    ...
    assert thr.deciding_observer == "kev"
    assert "kev" in dp.used_observers

    ap = store.axis_posture(conn, "demo-host", "threat")
    assert ap["status"] == "targeted"
    assert ap["deciding_observer"] == "kev"
...
def test_engine_threat_axis_unknown_when_observer_no_ops():
    """No candidates (or candidates with no overlay) -> the observer no-ops, so
    ...
    assert "kev" not in dp.used_observers


def test_engine_default_demo_device_threat_stays_unknown():
    """The shipped demo device has no cve_candidates/kev fields -> the observer
