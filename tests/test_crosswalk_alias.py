...
The defining property of the alias↔alias spine: a defect with NO cve (an OSV or
GHSA record that was never assigned a CVE) still anchors as a first-class peer,
and its equivalence to other defect_ids resolves correctly in both directions via
the symmetric edges `add_defect_alias` writes.
"""
from posture import store, spine


def test_cveless_defect_anchors_directly():
    """An OSV record with no CVE is upserted as a catalog row keyed by its own
    defect_id (the defects PK is TEXT, any scheme) — no cve required to exist."""
    conn = store.connect(":memory:")
    store.upsert_defect(conn, {
        "id": "OSV-2026-42", "defect_type": "osv", "published": "2026-05-01",
        "description": "an osv record with no cve alias",
        "source": "osv", "fetched_at": "t", "policy_version": "v", "complete": 1,
    })
    conn.commit()
    row = store.get_defect(conn, "OSV-2026-42")
    assert row is not None
    assert row["defect_type"] == "osv"
    # and it is reachable via the defect-type registry (posture spine show)
    counts = {r["defect_type"]: r["n"] for r in store.defect_type_counts(conn)}
    assert counts.get("osv") == 1


def test_cveless_defect_equivalence_resolves_both_directions():
...
def test_three_way_equivalence_is_transitive_via_edges():
    """cve <-> ghsa <-> osv: three peers of one defect. Each pair gets symmetric
...
def test_add_defect_alias_is_idempotent():
    conn = store.connect(":memory:")
