"""The uniform observer contract — the heart of the skeleton/flesh split.

Every source (NVD, a future CIS checker, a Shodan feed, an SBOM reader) is
one module implementing `Observer`. The engine is source-agnostic: it never
imports a source by name, it asks the registry for the observers a policy
...
class Verdict:
    ...
    so a Verdict is only ever created when a observer actually had something to
...
# ---------------------------------------------------------------------------
# ObserverResult — what a observer returns from assess()
# ---------------------------------------------------------------------------

@dataclass
class ObserverResult:
    """The output of one observer's assessment of one device.

    `verdicts` carry their observer id in provenance (the engine fills
    policy_version/fetched_at/complete if the observer left them partial).
...
# ---------------------------------------------------------------------------
# Observer — the uniform contract every source implements
# ---------------------------------------------------------------------------
...
class Observer(ABC):
    ...
    active policy may override `bias`, `weight`, and `order` per-observer.

    `assess(device, policy)` fetches + interprets for one device and returns a
    ObserverResult. The engine times the call (for health), stamps provenance,
    records a health sample, and commits through the completeness gate — so a
    observer implementation only needs to: fetch -> decide per record -> emit
    Verdicts with provenance.observer set.
    """

    id: str
    axes: tuple[Axis, ...]
    bias: str = "neutral"  # default; policy may override
    key_kind: str | None = None  # the identifier kind this observer emits (e.g. "cve")

    @abstractmethod
    def assess(self, device: dict, policy) -> ObserverResult:
        """Assess one device. Return Verdicts on this observer's axes.

        The implementation should set `verdict.provenance.observer = self.id`
        and may set `raw_ref`. The engine fills policy_version / fetched_at /
        complete (from the fetch) if the observer leaves them unset.

        `key_kind` (class-level) declares the identifier kind of the keys this
        observer emits, so the vocab monitor can surface a NEW kind as a
        candidate term (auto) without crashing. Leave None if the observer
        emits no joinable identifier.
        """
        raise NotImplementedError

    # convenience for subclasses: stamp a partial provenance with this observer
...
# ---------------------------------------------------------------------------
# ObserverRegistry — the uniform socket sources plug into
# ---------------------------------------------------------------------------

@dataclass
class ObserverRegistry:
    """The set of observers available to the engine, keyed by id.

    The engine asks `for_axis(axis, policy)` for the observers a policy
    ...
    _by_id: dict[str, Observer] = field(default_factory=dict)

    def register(self, w: Observer) -> None:
        if w.id in self._by_id:
            raise ValueError(f"observer already registered: {w.id}")
        self._by_id[w.id] = w

    def get(self, observer_id: str) -> Observer | None:
        return self._by_id.get(observer_id)

    def all(self) -> list[Observer]:
        return list(self._by_id.values())

    def for_axis(self, axis, policy) -> list[Observer]:
        """Observers authorized by the policy for `axis`, in authority order:
        LOWER `order` = HIGHER authority, so the lowest-order observer runs LAST
        and overrides the others (priority 1 is the top authority). Ties broken
        by observer id for determinism.

        Axes match by string VALUE, so a promoted non-seed axis (a string, not
        an Axis enum) still matches a observer that declares it — the dimension
        set grows without the enum being the law.
        """
        ax = axis.value if isinstance(axis, Axis) else str(axis)
        authorized: list[Observer] = []
        for w in self._by_id.values():
            w_axes = [a.value if isinstance(a, Axis) else str(a) for a in w.axes]
            if ax not in w_axes:
                continue
            if not policy.has_observer(w.id):
                continue
            authorized.append(w)
        order_of = lambda wid: policy.observer_order(wid)
    ...
    def used_in(self, verdicts: Iterable[Verdict]) -> list[str]:
        """Distinct observer ids that actually produced a verdict — for
        attribution (only attribute sources that were actually consulted)."""
        seen: list[str] = []
        for v in verdicts:
            wid = v.provenance.observer if v.provenance else ""
