...
| **Apple** | **HTML scrape only, no API.** Cleanest JSON = community **SOFA feed** `sofa.macadmins.io/v2/{macos,ios,…}_data_feed.json` (6h, <1000 req/h) — a middleman witness needing its own dossier | none (robots Request-rate 5/s for Inquira UA only) | legacy `HT######` / current 6-digit numeric; CVE- standard | PUBLIC |
